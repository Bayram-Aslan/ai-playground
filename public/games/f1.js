...
function startSequence() {
    ...
    timerInterval = setInterval(() => {
        lamps[count*2].classList.add('on'); lamps[count*2+1].classList.add('on');
        count++;
        if(count === 5) {
            clearInterval(timerInterval);
            state = 'WAIT';
            timerInterval = setTimeout(go, Math.random() * 3000 + 1000);
        }
    }, 800);
}
...
function log(label, val, isFail) {
    ...
    div.style.color = isFail ? "#ff1e1e" : "#aaa";
    div.innerHTML = `<span>${label}</span> <span>${val}</span>`;
    historyList.prepend(div);
}

function save(s) {
    top5.push({name: user, score: s});
...
function updateBoard() {
    leaderboard.innerHTML = top5.map((entry, i) => `
        <div class="list-item">
            ...
            <span class="neon-text">${entry.score.toFixed(3)}</span>
        </div>
    `).join('') || "<div class='list-item'>KAYIT YOK</div>";
}

function showReset() { trigger.style.display = "none"; resetBtn.style.display = "inline-block"; }

function resetUI() {
    state = 'IDLE';
    lamps.forEach(l => l.classList.remove('on'));
    timer.innerText = "0.000"; timer.style.color = "rgba(255,255,255,0.03)";
